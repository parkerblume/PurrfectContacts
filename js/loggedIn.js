...
function createTableRow(jsonObject)
{
    ...
    if (jsonObject != null)
    {
        ...
        let phoneItem = `<input onkeydown="phoneNumberFormat(this)" type="text" style="width:100%" placeholder="XXX-XXX-XXXX" value="${jsonObject.Phone}" required pattern="^[(]?[0-9]{3}[)]?[-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4}$">`;
    ...
    else
    {
        ...
        form = [
            ...
            '<input onkeydown="phoneNumberFormat(this)" type="text" style="width:100%" placeholder="XXX-XXX-XXXX" required pattern="^[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4}$">',
